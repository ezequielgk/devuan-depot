...
static bool bufdatabegin(struct wlr_buffer *buffer, uint32_t flags, void **data, uint32_t *format, size_t *stride);
...
cleanupmon(struct wl_listener *listener, void *data)
{
	...
	for (i = 0; i < LENGTH(m->pool); i++)
		if (m->pool[i]) wlr_buffer_drop(&m->pool[i]->base);

	if (m->drw) drwl_setimage(m->drw, NULL);
	if (m->drw) drwl_destroy(m->drw);
	...
	if (m->scene_buffer) wlr_scene_node_destroy(&m->scene_buffer->node);
	free(m);
}
...
dwl_ipc_manager_bind(struct wl_client *client, void *data, uint32_t version, uint32_t id)
{
	...
	zdwl_ipc_manager_v2_send_tags(manager_resource, 9);
...
focusclient(Client *c, int lift)
{
	...
	if (c && !client_is_unmanaged(c)) {
		...
		if (!exclusive_focus && !seat->drag)
			client_set_border_color(c, (float[])COLOR(colors[SchemeSel][2]));
	}

	/* If an overlay is focused, don't focus or activate the client,
	 * but only update its position in fstack to render its border with (float[])COLOR(colors[SchemeSel][2])
	...
	if (focused_client && focused_client != c && !(c && client_is_unmanaged(c))) {
		...
		if (c)
			client_set_border_color(focused_client, (float[])COLOR(colors[SchemeNorm][2]));
		if (s && s->mapped)
...
mapnotify(struct wl_listener *listener, void *data)
{
	...
	for (i = 0; i < 4; i++) {
		c->border[i] = wlr_scene_rect_create(c->scene, 0, 0,
				c->isurgent ? (float[])COLOR(colors[SchemeUrg][2]) : (float[])COLOR(colors[SchemeNorm][2]));
...
urgent(struct wl_listener *listener, void *data)
{
	...
	if (client_surface(c)->mapped)
		client_set_border_color(c, (float[])COLOR(colors[SchemeUrg][2]));
}
