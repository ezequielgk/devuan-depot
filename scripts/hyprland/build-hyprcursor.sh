...
echo "Getting latest hyprcursor tag..."
LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprcursor/releases | jq -r '.[].tag_name | select(contains("beta") | not)' | head -n 1)
if [ -z "$LATEST_TAG" ] || [ "$LATEST_TAG" = "null" ]; then
  # Fallback to tags if no releases exist
  LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprcursor/tags | jq -r '.[0].name')
fi

echo "Obtained latest version: $LATEST_TAG"

echo "Cloning hyprcursor..."
git clone --branch "$LATEST_TAG" --depth 1 https://github.com/hyprwm/hyprcursor.git src/hyprcursor
cd src/hyprcursor

echo "Compiling hyprcursor..."
...
Depends: ${DEPS:-libc6}
Maintainer: Zeke Ezequielgk <ezequieldtz@tuta.io>
Description: hyprcursor - Wayland compositor ecosystem
CTRL

dpkg-deb --build --root-owner-group pkgroot "hyprcursor_${VERSION}_amd64.deb"
mv "hyprcursor_${VERSION}_amd64.deb" ../..
