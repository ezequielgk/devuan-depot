...
echo "Getting latest aquamarine tag..."
LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/aquamarine/releases | jq -r '.[].tag_name | select(contains("beta") | not)' | head -n 1)
if [ -z "$LATEST_TAG" ] || [ "$LATEST_TAG" = "null" ]; then
  # Fallback to tags if no releases exist
  LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/aquamarine/tags | jq -r '.[0].name')
fi

echo "Obtained latest version: $LATEST_TAG"

echo "Cloning aquamarine..."
git clone --branch "$LATEST_TAG" --depth 1 https://github.com/hyprwm/aquamarine.git src/aquamarine
cd src/aquamarine

echo "Compiling aquamarine..."
...
Depends: ${DEPS:-libc6}
Maintainer: Zeke Ezequielgk <ezequieldtz@tuta.io>
Description: aquamarine - Wayland compositor ecosystem
CTRL

dpkg-deb --build --root-owner-group pkgroot "aquamarine_${VERSION}_amd64.deb"
mv "aquamarine_${VERSION}_amd64.deb" ../..
