...
echo "Getting latest hyprwayland-scanner tag..."
LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprwayland-scanner/releases | jq -r '.[].tag_name | select(contains("beta") | not)' | head -n 1)
if [ -z "$LATEST_TAG" ] || [ "$LATEST_TAG" = "null" ]; then
  # Fallback to tags if no releases exist
  LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprwayland-scanner/tags | jq -r '.[0].name')
fi

echo "Obtained latest version: $LATEST_TAG"

echo "Cloning hyprwayland-scanner..."
git clone --branch "$LATEST_TAG" --depth 1 https://github.com/hyprwm/hyprwayland-scanner.git src/hyprwayland-scanner
cd src/hyprwayland-scanner

echo "Compiling hyprwayland-scanner..."
...
Architecture: amd64
Description: hyprwayland-scanner stub
CTRLSTUB
...
Depends: ${DEPS:-libc6}
Maintainer: Zeke Ezequielgk <ezequieldtz@tuta.io>
Description: hyprwayland-scanner - Wayland compositor ecosystem
CTRL

dpkg-deb --build --root-owner-group pkgroot "hyprwayland-scanner_${VERSION}_amd64.deb"
mv "hyprwayland-scanner_${VERSION}_amd64.deb" ../..
