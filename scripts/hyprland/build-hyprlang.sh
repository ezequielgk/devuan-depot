...
echo "Getting latest hyprlang tag..."
LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprlang/releases | jq -r '.[].tag_name | select(contains("beta") | not)' | head -n 1)
if [ -z "$LATEST_TAG" ] || [ "$LATEST_TAG" = "null" ]; then
  # Fallback to tags if no releases exist
  LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/hyprwm/hyprlang/tags | jq -r '.[0].name')
fi

echo "Obtained latest version: $LATEST_TAG"

echo "Cloning hyprlang..."
git clone --branch "$LATEST_TAG" --depth 1 https://github.com/hyprwm/hyprlang.git src/hyprlang
cd src/hyprlang

echo "Compiling hyprlang..."
...
Depends: ${DEPS:-libc6}
Maintainer: Zeke Ezequielgk <ezequieldtz@tuta.io>
Description: hyprlang - Wayland compositor ecosystem
CTRL

dpkg-deb --build --root-owner-group pkgroot "hyprlang_${VERSION}_amd64.deb"
mv "hyprlang_${VERSION}_amd64.deb" ../..
