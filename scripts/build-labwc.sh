...
echo "Getting latest labwc tag..."
LATEST_TAG=$(curl -sL -H "Authorization: Bearer ${GITHUB_TOKEN}" https://api.github.com/repos/labwc/labwc/releases/latest | jq -r '.tag_name')
...
echo "Cloning labwc..."
git clone --branch "$LATEST_TAG" --depth 1 https://github.com/labwc/labwc.git src/labwc
cd src/labwc

echo "Compiling labwc..."
# Wrap mode nodownload forces it to use system/external wlroots.
meson setup build --buildtype=release --sysconfdir=/etc --strip -Db_lto=true -Db_ndebug=true -Db_pie=true --prefix=/usr --wrap-mode=nodownload
ninja -C build
...
Depends: ${DEPS}
Maintainer: Zeke Ezequielgk <ezequieldtz@tuta.io>
Description: labwc - A Wayland window-stacking compositor
 Labwc is a wlroots-based window-stacking compositor for Wayland,
 inspired by Openbox. It is lightweight and independent with a focus
 on simply stacking windows well and rendering Window decorations.
CTRL
...
dpkg-deb --build --root-owner-group pkgroot "labwc_${VERSION}_amd64.deb"
